    ...
    <div :class="{ 'blur-sm pointer-events-none select-none': isLoading }">
        ...
        <table class="w-full border-white overflow-scroll" v-if="favouriteMovies.length">
            <tr class="border-b">
                ...
                <th class=""></th>
                <th class="text-right py-1">Release Date</th>
            ...
            <tr v-for="(movie, index) in favouriteMoviesToList" :key="index" class="border-b my-1">
                ...
                <td>
                    <button v-if="isFavourite(movie.id)" @click="removeFromFavourites(movie.id)" class="flex flex-row justify-start w-12 mt-1">
                        <img src="@/assets/heart-filled.png" />
                    </button>
                    <button v-else @click="addToFavourites(movie)" class="flex flex-row justify-center w-12 mt-1">
                        <img src="@/assets/heart-empty.png" />
                    </button>
                </td>
                <td class="text-right">
                    <div v-if="movie.release_date">{{ moment(movie.release_date).format("DD/MM/YYYY") }}</div>
                    <div v-else>N/A</div>
                </td>
                <td class="text-right pr-5">
                    <span v-if="movie.vote_count !== 0">{{ movie.vote_average.toFixed(1) + "/10" }}</span>
                    <span v-else>N/A</span>
                </td>
                <td class="text-right pr-5">
                    <span v-if="movie.vote_count !== 0">{{ movie.vote_count }}</span>
                    <span v-else>N/A</span>
                </td>
            </tr>
        </table>
